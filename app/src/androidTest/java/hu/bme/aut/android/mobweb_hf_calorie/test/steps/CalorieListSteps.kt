...
class CalorieListSteps {
    ...
    fun the_displayed_data_is_correct() {
        onView(withId(R.id.rv_calorie))
            ...
            .check(
                matches(
                    MyMatcher.atPosition(
                        0,
                        hasDescendant(withText(getCurrentDateInDisplayFormat()))
                    )
                )
            )
            .check(
                matches(
                    MyMatcher.atPosition(
                        0,
                        hasDescendant(withText(getCurrentTimeInDisplayFormat()))
                    )
                )
            )
    }
    ...
    fun item_has_web_page() {
        onView(withText("Web")).perform(ViewActions.click())
